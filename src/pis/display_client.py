import argparse
import pis.display.display_types as types
from pis.display import timetable_view, split_view, platform_view, info_view
from pis.display import mqtt_connection as mqtt, display_printer as dp

parser = argparse.ArgumentParser(description='sets the correct display')
parser.add_argument('-view', type=str, help='enter a display view name', choices=['tableview', 'splitview', 'platformview', 'infoview'])
parser.add_argument('-s', type=str, help='enter station short code')
parser.add_argument('-p', type=str, help='enter platform number')
parser.add_argument('-transit', type=str, help='enter transit type', choices=['departures', 'arrivals'])
parser.add_argument('-transport', type=str, help='enter transport type', choices=['commuter', 'long_distance'])
